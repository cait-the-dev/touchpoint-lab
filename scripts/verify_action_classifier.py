...
from src.action_classifier import _norm, load_labels, predict, train
...
raw = pd.read_csv(RAW, usecols=["event_name"]).assign(
    event_name=lambda d: _norm(d["event_name"])
)
...
for nm in unique_names:
    ...
    df.loc[mask_train, ["event_name", "label"]].drop_duplicates().to_csv(
        tmp_labels, index=False
    )
    ...
    scores.append(f1_score(y_true, y_pred, pos_label="BRAND", zero_division=1))
