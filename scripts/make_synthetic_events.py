...
base_events = [
    "Opened Email", "Email Open", "Clicked Email",
    "Placed Order", "Viewed Product", "Active on Site",
    "Unsubscribed", "Clicked email to unsubscribe",
    "SMS Sent", "SMS Delivered",
    "Octane: quiz completed: Find the perfect eye shadow",
    "Black Crow Identified",
    "Viewed Product XYZ", "blotout viewed product XYZ"
]
...
for _ in range(n_rows):
    rows.append({
        ...
        "event_ts": start + datetime.timedelta(
            seconds=random.randint(0, 60*60*24*120)),
        "event_name": random.choice(events),
        "purchased_7d": 1 if "Order" in events else np.random.binomial(1, .05)
    })
